...
RANDOM_SEED  = 42
TRAIN_RATIO  = 0.70   # 70/30 chronological train/test split
MIN_REGIME_DURATION = 3   # months — runs shorter than this are absorbed into neighbours
...
# Use JSON encoder
# For scalar and array types. Also converts NaN to null.
...
# Good practice for scalar/Python float NaN conversion for JSON
def _safe(v):
    if v is None:
        return None
...
# Load & Rename:
# Load FRED CSV, parse dates, and renaming the columns.
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Forward-fill columns (no backfill - this will avoid bias)
...
# Scale Features
# Drop rows with NaN in columns, return cleaned df without scaling.
def scale_features(df: pd.DataFrame, feature_cols: list) -> pd.DataFrame:
    return df.dropna(subset=feature_cols).copy()


# Train/Test Split
# Chronological 70/30 split. Scaler fit on train only to prevent data leakage.
def split_and_scale(df_model: pd.DataFrame, feature_cols: list, train_ratio: float = TRAIN_RATIO):
    n_train      = int(len(df_model) * train_ratio)
    ...
    X_full       = scaler.transform(df_model[feature_cols].values)

    return X_train, X_test, X_full, scaler, df_train, df_test


# Train HMM (will need multiple restarts)
def fit_hmm_best(X_scaled: np.ndarray, n_components: int = N_REGIMES,
    ...
    with warnings.catch_warnings():
        ...
        for i in range(n_restarts):
            model = GaussianHMM(
                ...
                min_covar = 1e-2, # Check this if we want to keep it.
            )
            try:
                model.fit(X_scaled)
    ...
    if best_model is None:
        raise RuntimeError("All HMM restarts failed: Check data.")
...
def decode_states(model: GaussianHMM, X_scaled: np.ndarray):
    ...
    state_probs   = model.predict_proba(X_scaled)    # shape (n, 3) - Keep refining
    return hidden_states, state_probs


# Minimum duration smoother
# Any regime run shorter than min_duration months is replaced by the preceding state.
# Iterates until no short runs remain (handles back-to-back short runs).
def smooth_regimes(hidden_states: np.ndarray, min_duration: int = MIN_REGIME_DURATION) -> np.ndarray:
    ...
    while changed:
        ...
        while i < len(states):
            ...
            if run_len < min_duration:
                fill = states[i - 1] if i > 0 else states[j]   # use successor at start
                states[i:j] = fill
                changed = True
            i = j
    return states
...
# Diagnostics check for testing - Not needed for viz
...
# Max drawdown helper
# Max peak-to-trough drawdown from a series of period returns. 
...
def compute_state_statistics(df_model: pd.DataFrame,
    ...
    # The sort is by regime id
    stats_list.sort(key=lambda x: x["regime_id"])
    return stats_list

# Transition matrix construction
# Reordered to match output regime IDs (0=Growth, 1=Crisis, 2=Transition)
...
def build_output_json(df: pd.DataFrame, df_model: pd.DataFrame,
                      ...
                      scaler: StandardScaler, feature_cols: list,
                      log_likelihood: float, train_cutoff_date) -> dict:
    # Null rows not used
    key_to_id  = {r["key"]: r["id"] for r in REGIME_META}
    key_to_lbl = {r["key"]: r["label"] for r in REGIME_META}
    ...
    # Probs are reordered to match regime IDs
    key_to_id = {r["key"]: r["id"] for r in REGIME_META}
    ...
    for i, (idx, _) in enumerate(df_model.iterrows()):
        ...
        regime_by_date[idx] = {
            ...
            "regime_probs": probs,
            "split":        "train" if idx <= train_cutoff_date else "test",
        }
    ...
    for date, row in df.iterrows():
        ...
        if date in regime_by_date:
            ...
            entry.update({"regime": None, "regime_label": None, "regime_probs": None, "split": None})
    ...
    return {
        "metadata": {
            ...
            "train_date_range": {
                "start": df_model.index[0].strftime("%Y-%m-%d"),
                "end":   train_cutoff_date.strftime("%Y-%m-%d"),
            },
            "test_date_range": {
                "start": df_model.index[df_model.index > train_cutoff_date][0].strftime("%Y-%m-%d"),
                "end":   df_model.index[-1].strftime("%Y-%m-%d"),
            },
...
if __name__ == "__main__":
    ...
    df_model = scale_features(df_clean, FEATURE_COLS)

    X_train, X_test, X_full, scaler, df_train, df_test = split_and_scale(df_model, FEATURE_COLS)
    train_cutoff_date = df_train.index[-1]

    print(f"Train: {df_train.index[0].date()} - {df_train.index[-1].date()}  ({len(df_train)} months)")
    print(f"Test : {df_test.index[0].date()}  - {df_test.index[-1].date()}  ({len(df_test)} months)")

    # Train on train split only
    model, log_likelihood = fit_hmm_best(X_train)

    state_label_map = label_regimes(model, scaler, FEATURE_COLS)

    # Decode full sequence (train + test) with the trained model
    hidden_states, state_probs = decode_states(model, X_full)
    hidden_states = smooth_regimes(hidden_states)
